...
def vec_carrera(new_doc):
    ...
    filename= 'carrera_model'

    modelo_carrera = pickle.load(open(filename,'rb'))
    
    
    
    
    if (modelo_carrera.predict_proba(X_new).max())>0.5:
        return(modelo_carrera.predict(X_new)[0])
