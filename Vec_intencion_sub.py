...
def vec_intents(new_doc):
    ...
    df_textos = pd.read_csv('data_intents.csv', sep=';', encoding = "ANSI")
    df_textos = Corregir_Documentos(df_textos,['oracion'],False,True,True)
    

    #2. Modelizar los documentos de df_textos
    vectorizador, df_textos = Generar_Matriz_Tfidf(df_textos,'oracion',ngram=(-1,1))
    

    #3. Separar el corpus en Train/Test
    
    X = df_textos.drop(['intencion','subIntencion'],axis=1)
    y = [ra + '#' + rb for (ra,rb) in list(zip(df_textos['intencion'],df_textos['subIntencion']))]
    ...
    filename= 'intents_model'

    modelo_intents = pickle.load(open(filename,'rb'))
    
    
    
    if (modelo_intents.predict_proba(X_new).max())>0.5:
        return(modelo_intents.predict(X_new)[0])
    
    return('generalidades#todas')
